...
const CustomAnimeCard = ({ anime, type }) => {
    ...
    return (type == "Saved" ?
        ...
        <div className="relative min-h-[120px] max-h-[120px] min-w-[90px] sm:min-h-[200px] sm:max-h-[250px] sm:min-w-[190px] group ml-1 sm:mb-1 mb-1 mt-1 sm:mt-2 cursor-pointer">
            <div className='md:text-lg w-fit rounded-none rounded-bl-sm bg-secondary/50 group-hover:bg-secondary/90 text-sm p-0 absolute top-0 right-0 z-30'><RemoveAnimeButton animeId={animeId} /></div>
            <Link href={`/watch/${encodeURIComponent(animeId)}?ep=${episodeNumber}`} className="w-fit">
                <div className='text-lg text-white absolute top-1 left-2 font-semibold z-30 textStroke'>EP{episodeNumber}</div>
                <div className="absolute inset-0 h-full rounded-sm bg-gradient-to-b from-gray-200/0 via-gray-900/20 group-hover:via-gray-900/30 to-gray-950/70 group-hover:to-gray-950/70 "></div>
                <div className="absolute inset-0 h-full rounded-sm bg-gradient-to-tl from-gray-200/0 via-gray-900/10 group-hover:via-gray-900/0 to-gray-950/20 group-hover:to-gray-950/50 "></div>
                ...
                <Image
                    className={cn("min-h-[120px] max-h-[120px] min-w-[90px] sm:min-h-[200px] sm:max-h-[250px] sm:min-w-[190px] object-cover rounded-sm lg:hover:drop-shadow-lg")}
                    alt={name}
                    width={300}
                    height={400}
                    layout="responsive"
            ...
            <div className='top-0 bg-slate-500'>
